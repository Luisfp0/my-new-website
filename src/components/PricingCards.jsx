const PricingCards = (props) => {
  ...
  const renderCard = (cardData) => {
    return cardData.map((card, index) => (
      <div
        key={index}
        className={`w-full shadow-lg flex flex-col items-start px-[36px] pt-[34px] pb-[32px] relative lg:px-[36px] lg:pt-[34px] lg:pb-[32px] md:px-[15px] md:pt-[15px] md:pb-[20px] ${
          card.title === "Premium" &&
          "bg-[#292929] border-[1px] border-[#474747] rounded-lg"
        }`}
      >
        {card.title === "Premium" && (
          <h1 className="px-[6px] absolute py-[2px] text-[12px] bg-[#00000033] rounded-md">
            Most Popular
          </h1>
        )}
  ...
  return (
    <div
      className={`lg:max-w-[1200px] md:max-w-[960px] sm:max-w-[720px] lg:flex md:flex mx-auto gap-[8px] text-white`}
    >
      {props.activeTab === "monthly"
        ? renderCard(cardMonthlyData)
        : renderCard(cardYearlyData)}
    </div>
  );
};
